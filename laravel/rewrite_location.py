...
# Add scaffold methods to scaffoldTaxonomyPages
scaffold_replacements = """    public function scaffoldTaxonomyPages(bool $replace = false): array
    {
        return [
...
content = re.sub(r'    public function scaffoldTaxonomyPages\(bool \$replace = false\): array\n    \{.*?\}', scaffold_replacements, content, flags=re.DOTALL)

# Add scaffoldCities and scaffoldServiceCities
scaffolding_methods = """
...
content = re.sub(r'    private function scaffoldServiceCategories', scaffolding_methods + "\n    private function scaffoldServiceCategories", content)

# Rewrite buildLocationsHub
new_buildLocationsHub = """    private function buildLocationsHub(): array
    {
...
content = re.sub(r'    private function buildLocationsHub\(\): array\n    \{.*?(?=    private function buildPortfolioIndex|\Z)', new_buildLocationsHub + "\n\n", content, flags=re.DOTALL)


# Add buildCity and buildServiceCity
new_build_methods = """
...
content = re.sub(r'(?=\}\s*$)', new_build_methods, content)

with open(file_path, 'w') as f:
    f.write(content)
print("Updated locations hub, city, and service city methods successfully.")
