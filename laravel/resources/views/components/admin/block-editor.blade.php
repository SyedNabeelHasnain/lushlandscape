                ...
                <div x-show="block._open" x-cloak x-collapse class="divide-y divide-gray-50">
                    <div class="p-5 space-y-4">
                        ...
                        <template x-if="block.is_layout_section">
                            <div class="space-y-4">
                                {{-- Hero / Service Hero --}}
                                <template x-if="['hero', 'service_hero', 'scp_hero'].includes(block.block_type)">
                                    <div class="space-y-3">
                                        <div class="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-forest/10 bg-forest/5 px-3 py-2">
                                            <div>
                                                <p class="text-xs font-semibold text-gray-700">Dynamic Variables</p>
                                                <p class="mt-0.5 text-[10px] text-gray-400">Click into any hero text field, then insert a variable from the current page context.</p>
                                            </div>
                                            <div class="relative" x-data="{ dynOpen: false, dynSearch: '' }">
                                                <button type="button"
                                                    @click="dynOpen = !dynOpen; if (dynOpen) { $nextTick(() => $refs.layoutDynamicVariableSearch?.focus()); }"
                                                    class="flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 text-[10px] font-medium text-forest shadow-sm transition hover:bg-forest-50">
                                                    <i data-lucide="database" class="h-3 w-3"></i> Insert Variable
                                                </button>
                                                <div x-show="dynOpen" @click.outside="dynOpen = false" x-cloak
                                                    class="absolute right-0 z-50 mt-2 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-gray-100 bg-white shadow-xl">
                                                    <div class="border-b border-gray-100 p-2">
                                                        <input x-ref="layoutDynamicVariableSearch" type="text" x-model="dynSearch"
                                                        ...
                                                        <template x-for="group in filteredDynamicVariableGroups(dynSearch)" :key="group.key">
                                                            <div class="space-y-1">
                                                                ...
                                                                <template x-for="variable in group.variables" :key="variable.token">
                                                                    <button type="button"
                                                                        @click="window.insertVariableTokenIntoFocusedField(variable.token); dynOpen = false; dynSearch = ''"
                                        ...
                                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                            <div class="col-span-2">
                                                <label class="block text-xs font-semibold text-gray-600 mb-1">Heading
                                                    Override <span class="font-normal opacity-70">(blank =
                                                        auto)</span></label>
                                                <input type="text" x-model="block.content.heading"
                                                    class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                            </div>
                                            <div class="col-span-2">
                                                <label class="block text-xs font-semibold text-gray-600 mb-1">Subtitle
                                                    Override</label>
                                                <textarea x-model="block.content.subtitle" rows="2"
                                                    class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30"></textarea>
                                            </div>
                                            <div class="col-span-2">
                                                <label class="block text-xs font-semibold text-gray-600 mb-1">Eyebrow Label</label>
                                                <input type="text" x-model="block.content.eyebrow"
                                                    class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-semibold text-gray-600 mb-1">CTA
                                                    Text</label>
                                                <input type="text" x-model="block.content.cta_text"
                                                    class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-semibold text-gray-600 mb-1">CTA
                                                    URL</label>
                                                <input type="text" x-model="block.content.cta_url"
                                                    class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                            </div>
                                        </div>
                                        <div class="space-y-3 border-t border-gray-100 pt-3">
                                            <div>
                                                <label class="mb-1 block text-xs font-semibold text-gray-600">Primary Hero Image</label>
                                                @include('components.admin.partials.block-media-picker', [
                                                    'fieldKeyExpression' => "'hero_media_id'",
                                                    'modelExpression' => 'block.content.hero_media_id',
                                                ])
                                            </div>
                                            <div>
                                                <label class="mb-1 block text-xs font-semibold text-gray-600">Slider Images</label>
                                                @include('components.admin.partials.block-media-multi-picker', [
                                                    'fieldKeyExpression' => "'extra_image_ids'",
                                                    'modelExpression' => 'block.content.extra_image_ids',
                                                    'emptyText' => 'No slider images selected',
                                                ])
                                                <p class="mt-2 text-[10px] text-gray-400">If no background video is set, the primary hero image becomes slide 1 and these images continue the slider.</p>
                                            </div>
                                            <div>
                                                <label class="mb-1 block text-xs font-semibold text-gray-600">Background
                                                    Video URL <span class="font-normal opacity-70">(MP4, YouTube, or Vimeo)</span></label>
                                                <input type="text" x-model="block.content.video_url"
                                                    placeholder="https://www.youtube.com/watch?v=..."
                                                    class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-forest/30">
                                            </div>
                                        </div>
                                    </div>
                                </template>
                                ...
                                <template x-if="block.block_type === 'process_steps'">
                                    <div class="space-y-3">
                                        <label class="block text-xs font-semibold text-gray-600 mb-1">Section
                                            Heading</label>
                                        <input type="text" x-model="block.content.heading"
                                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                        <div class="space-y-2">
                                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                                                Steps</p>
                                            <template x-for="(step, si) in (block.content.steps || [])" :key="si">
                                                <div
                                                    class="bg-gray-50 border border-gray-100 rounded-xl p-3 space-y-2 relative">
                                                    <div class="flex items-center gap-3">
                                                        <span
                                                            class="w-5 h-5 rounded-full bg-forest text-white text-[10px] flex items-center justify-center font-bold"
                                                            x-text="si+1"></span>
                                                        <input type="text" x-model="step.title" placeholder="Step title"
                                                            class="flex-1 px-3 py-1.5 text-xs border border-gray-200 rounded-lg">
                                                    </div>
                                                    <textarea x-model="step.desc" placeholder="Description" rows="2"
                                                        class="w-full px-3 py-1.5 text-xs border border-gray-200 rounded-lg resize-none"></textarea>
                                                </div>
                                            </template>
                                        </div>
                                    </div>
                                ...
                                {{-- FAQ Section --}}
                                <template x-if="block.block_type === 'faq_section'">
                                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                        <div class="col-span-2">
                                            <label class="block text-xs font-semibold text-gray-600 mb-1">Heading
                                                Override</label>
                                        ...
                                        <div>
                                            <label class="block text-xs font-semibold text-gray-600 mb-1">Max
                                                Items</label>
                                            <input type="number" x-model="block.content.limit"
                                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-semibold text-gray-600 mb-1">Category
                                                ID</label>
                                            <input type="number" x-model="block.content.faq_category_id"
                                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                        </div>
                                    </div>
                                </template>
                                ...
                                {{-- CTA Section --}}
                                <template x-if="block.block_type === 'cta_section'">
                                    <div class="space-y-3">
                                        <div>
                                        ...
                                        <div>
                                            <label
                                                class="block text-xs font-semibold text-gray-600 mb-1">Subtitle</label>
                                            <textarea x-model="block.content.subtitle" rows="2"
                                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30"></textarea>
                                        </div>
                                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                            <input type="text" x-model="block.content.button_text"
                                                placeholder="Button Text"
                                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg">
                                            <input type="text" x-model="block.content.button_url"
                                ...
                                {{-- Catch-all for others / No settings --}}
                                <template
                                    x-if="['services_grid', 'stats_bar', 'local_about', 'local_intro', 'trust_badges', 'service_body', 'city_availability', 'portfolio_gallery', 'portfolio_preview', 'benefits_grid', 'city_grid', 'blog_strip'].includes(block.block_type)">
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 mb-1">Heading Override
                                            <span class="font-normal opacity-70">(blank = auto)</span></label>
                                        <input type="text" x-model="block.content.heading"
                                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-forest/30">
                                        <p class="mt-2 text-[10px] text-gray-400 italic">This is an integrated layout
                                            section. Most content is managed via its own dedicated admin module or
                                            driven by database records.</p>
                                    </div>
                                </template>
                            </div>
                        </template>

                        {{-- REGULAR CONTENT BLOCK FIELDS --}}
                        <template x-if="!block.is_layout_section">
                            <div class="space-y-4">
