                ...
                <form @submit.prevent="submitWizard" id="luxury-wizard-form" class="flex-1 flex flex-col justify-between" :class="{ 'opacity-50 pointer-events-none': isSubmitting }">
                    ...
                    <div x-show="step === 1" x-transition.opacity.duration.500ms class="space-y-8">
                        ...
                        <div class="field-wrapper">
                            <select id="consultation_service" x-model="formData.service" class="field-ui pr-10" :class="formData.service ? 'has-value' : ''" required>
                            ...
                            <label for="consultation_service" class="field-label">Project Scope *</label>
                            <i data-lucide="chevron-down" aria-hidden="true" class="absolute right-0 top-3 w-4 h-4 text-forest/40 pointer-events-none"></i>
                        </div>

                        <div class="field-wrapper">
                            <select id="consultation_city" x-model="formData.city" class="field-ui pr-10" :class="formData.city ? 'has-value' : ''" required>
                                <option value="" disabled selected hidden></option>
                            ...
                            <label for="consultation_city" class="field-label">City *</label>
                            <i data-lucide="chevron-down" aria-hidden="true" class="absolute right-0 top-3 w-4 h-4 text-forest/40 pointer-events-none"></i>
                        </div>
                    </div>
                    ...
                    <div x-show="step === 2" x-transition.opacity.duration.500ms class="space-y-8" style="display: none;">
                        ...
                        <div class="field-wrapper">
                            <select id="consultation_property_type" x-model="formData.property_type" class="field-ui pr-10" :class="formData.property_type ? 'has-value' : ''">
                                <option value="" disabled selected hidden></option>
                            ...
                            <label for="consultation_property_type" class="field-label">Property Type</label>
                            <i data-lucide="chevron-down" aria-hidden="true" class="absolute right-0 top-3 w-4 h-4 text-forest/40 pointer-events-none"></i>
                        </div>

                        <div class="field-wrapper">
                            <textarea id="consultation_project_details" x-model="formData.project_details" class="field-ui min-h-[120px] resize-none" :class="formData.project_details ? 'has-value' : ''" placeholder=" " required></textarea>
                            <label for="consultation_project_details" class="field-label">Project Summary *</label>
                        </div>
                    </div>
                    ...
                    <div x-show="step === 3" x-transition.opacity.duration.500ms class="space-y-8" style="display: none;">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="field-wrapper">
                                <input id="consultation_full_name" type="text" x-model="formData.full_name" class="field-ui" :class="formData.full_name ? 'has-value' : ''" placeholder=" " required>
                                <label for="consultation_full_name" class="field-label">Full Name *</label>
                            </div>
                            <div class="field-wrapper">
                                <input id="consultation_phone" type="tel" x-model="formData.phone" class="field-ui" :class="formData.phone ? 'has-value' : ''" placeholder=" " required>
                                <label for="consultation_phone" class="field-label">Phone Number *</label>
                            </div>
                        </div>

                        <div class="field-wrapper relative">
                            <input id="consultation_email" type="email" x-model="formData.email" @blur="checkEmail" class="field-ui pr-24" :class="formData.email ? 'has-value' : ''" placeholder=" " :readonly="emailVerified" required>
                            <label for="consultation_email" class="field-label">Email Address *</label>
                            
                            <button type="button" x-show="showVerifyBtn && !showOtpField && !emailVerified" @click="sendOtp" class="absolute right-0 top-2.5 rounded-sm bg-forest px-4 py-1.5 text-[9px] font-bold uppercase tracking-[0.15em] text-white hover:bg-accent transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-forest">Verify</button>
                            <span x-show="emailVerified" class="absolute right-0 top-3 text-[10px] font-bold uppercase tracking-[0.1em] text-forest-light flex items-center gap-1"><i data-lucide="check-circle-2" aria-hidden="true" class="w-3 h-3"></i> Verified</span>
                        </div>

                        <div x-show="showOtpField" x-collapse class="bg-forest/5 p-6 border border-forest/10 mt-4 relative">
                            <label for="consultation_otp" class="text-xs text-forest mb-4 font-medium block">Please enter the 6-digit code sent to your email.</label>
                            <div class="flex gap-4">
                                <input id="consultation_otp" type="text" x-model="otpCode" maxlength="6" class="field-ui !bg-white !border-forest/20 text-center tracking-[0.5em] font-bold text-lg" placeholder="••••••">
                                <button type="button" @click="verifyOtp" class="btn-solid px-6 text-xs uppercase tracking-widest focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-forest" :disabled="otpCode.length !== 6">Confirm</button>
                            </div>
