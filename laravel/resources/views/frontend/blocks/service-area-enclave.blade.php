    ...
    @if($presentationMode === 'tabbed-enclave')
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-24">
            <div class="w-full lg:w-5/12 text-left">
                ...
                <h2 class="text-h2 font-heading font-bold text-ink mb-6 text-balance">{!! $heading !!}</h2>
                <div class="w-12 h-px bg-forest/30 mb-8"></div>
                <p class="text-body-lg text-text-secondary leading-relaxed">{!! $supportCopy !!}</p>
            </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 lg:gap-6">
                    @foreach($data as $city)
                        <a href="{{ url('areas/'.$city->slug) }}" class="group flex flex-col items-center justify-center p-6 rounded-[1.5rem] bg-white border border-stone hover:border-forest/40 hover:shadow-luxury hover:-translate-y-1 transition-all duration-500">
                            ...
                            <span class="text-sm font-bold uppercase tracking-[0.1em] text-ink text-center">{{ $city->name }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
